...
def get_onedrive_client_from_console(savesession):
	...
	onedriveclient = onedrivesdk.get_default_client(client_id = ONEDRIVEID, scopes=['wl.signin', 'wl.offline_access', 'onedrive.readwrite'])
	if savesession == True:
...
def read_dropbox_directory_from_console(client):
	temp_save_path = ""
	while temp_save_path == "":
		temp_save_path = raw_input("Please enter the absolute path to the folder in your Dropbox where I should save your downloaded files:\n")
		try:
...
def read_onedrive_directory_from_console(client):
	temp_path = ""
	while temp_path == "":
		temp_path = raw_input("Please enter the absolute path to the folder on OneDrive where I should save your downloaded files:\n")
...
def read_drive_directory_from_console(service):
	...
	id = ""
	while temp_save_path == "":
		temp_save_path = raw_input("Please enter the absolute path to the folder on your Google Drive where I should save your downloaded files:\n")
		if any(c in temp_save_path for c in FORBIDDEN_CHARS_PATH if c in temp_save_path):
			print "Your selected path contains one or more of these unallowed characters: " + str(FORBIDDEN_CHARS_PATH)
			temp_save_path = ""
		else:
			try:
				...
				while exists == True and index < len(foldernames) - 1:
					index += 1
					exists = False
					response = drive_service.files().list(q = "mimeType = 'application/vnd.google-apps.folder' and '" + id + "' in parents").execute()
					for file in response.get('files', []):
						if file.get('name') == foldernames[index]:
							id = file.get('id')
							exists = True
...
def handle_error(e):
	...
	if createlogfile == "True":
		print "For further information look at the logifle " + path.join(os.path.dirname(os.path.realpath(sys.argv[0])), LOGFILENAME)
	else:
	...
	if wheretosave == "Local":
		...
		for f in response.get('files', []):
			if f.get('name') == directoryname:
				...
				break;
		if exists == False:
			folder_metadata = {'name' : directoryname, 'mimeType' : 'application/vnd.google-apps.folder', 'parents' : [save_path]}
...
if version[0] == 2:
	if version[1] == 7:
		if version[2] >= 9:		# Script should work for version 2.7.9 and greater
			...
			print "This script was written for Python 2.7.11, your current version is " + str(version[0]) + "." + str(version[1]) + "." + str(version[2]) 
	...
	if paraminput[1] == "-c":
		if not os.path.exists(configfilepath):
			...
			if wheretosave == "Local":
				...
				yesorno = raw_input( "Would you like to change the path for the downloaded files on Google Drive? <y/n>\n")
			elif wheretosave == "OneDrive":
				yesorno = raw_input( "Would you like to change the path for the downloaded files on OneDrive? <y/n>\n")
			elif wheretosave == "Dropbox":
				yesorno = raw_input( "Would you like to change the path for the downloaded files in your Dropbox? <y/n>\n")
			if yesorno == 'y':
				if wheretosave == "Local":
		...
		if not os.path.exists(configfilepath):
			...
			if yesorno == 'y':
				...
				if enableordisable == "enable":
					print "You can now find the logfile here: " + path.join(os.path.dirname(os.path.realpath(sys.argv[0])), LOGFILENAME)
			elif yesorno == 'n':
				print "Aborted\n"
			else:
				print "Invalid input\n"
		...
		if not os.path.exists(configfilepath):
			...
			yesorno = raw_input( "You are currently using the " + nameofservice + ". Would you like to change your service?<y/n>\n")
			if yesorno == 'y':
				wheretosave_input = raw_input("Where should I save your downloaded files? <l/o/g/d>\n - on local harddrive (l)\n - on OneDrive (o)\n - on Google Drive (g)\n - in your Dropbox (d)\n")
...
try:
	...
	r = requests.get("https://studip.uni-passau.de/studip/index.php?again=yes&sso=shib", cookies = cookie_seminar_session, allow_redirects = False)
	...
	r = requests.post("https://sso.uni-passau.de/idp/Authn/UserPassword", cookies = sibboleth_cookies, data = payload, allow_redirects = False)
...
try:
	...
	r = requests.post("https://studip.uni-passau.de/Shibboleth.sso/SAML2/POST", cookies = cookie_shibstate, data = payload, allow_redirects = False)
		...
		while index_tr_open >= 0:
			...
			if tr.find('https://studip.uni-passau.de/studip/assets/images/icons/red/new/files.svg') >= 0:
				...
				if len(linktonewfiles) > 0:
					r = requests.get(linktonewfiles, cookies = cookies_seminar_session_and_shibsession, allow_redirects = True)
					...
					r = requests.get(downloadlink, cookies = cookies_seminar_session_and_shibsession, allow_redirects = True)
	...
	# Logout
	r = requests.get("https://studip.uni-passau.de/studip/logout.php?sso=shib", cookies = cookies_seminar_session_and_shibsession, allow_redirects = True, headers={'Connection':'close'})
	r.connection.close()
